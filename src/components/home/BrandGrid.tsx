    ...
    <section className="py-16 sm:py-24 bg-white">
      <div className="container-page">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Link href="/brands" className="text-xs font-semibold text-brand-navy border-b border-brand-red pb-0.5 shrink-0">
            ดูแบรนด์ทั้งหมด →
          </Link>
        </div>

        <ScrollFx effect="fade-up" stagger={0.08} className="mt-10 sm:mt-14 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3 sm:gap-4">
          {groups.map((g) => {
              ...
              return (
                <Link
                  key={g.key}
                  href={g.href}
                  className="group relative flex flex-col overflow-hidden rounded-2xl aspect-[3/4]"
                  style={{ backgroundColor: g.colorHex }}
                >
                  <div className="absolute inset-0 bg-gradient-to-b from-black/0 via-black/10 to-black/65" aria-hidden="true" />
                  ...
                  <div className="relative z-10 px-2.5 pb-2.5 pt-1.5 text-center">
                    ...
                    <span className="mt-2 inline-flex w-full items-center justify-center rounded-full bg-white text-brand-navy text-[10px] sm:text-[11px] font-bold py-2 group-hover:bg-brand-red group-hover:text-white transition-colors">
                      ทดลองขับ
                    </span>
                  </div>
                </Link>
